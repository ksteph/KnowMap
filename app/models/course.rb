class Course < ActiveRecord::Base
  ...
  has_many :nodes, :finder_sql => proc { "WITH RECURSIVE subgraphs AS ( SELECT subgraph_id FROM graph_membership_graphs WHERE graph_id = #{graph.id} UNION ALL SELECT g.subgraph_id FROM graph_membership_graphs g, subgraphs sg WHERE g.graph_id = sg.subgraph_id) SELECT n.*, ni.row AS row, ni.index AS index FROM nodes n LEFT JOIN node_indices ni ON n.id = ni.node_id WHERE n.id IN (SELECT DISTINCT m.node_id FROM graph_membership_nodes m, subgraphs sg WHERE m.graph_id = sg.subgraph_id OR m.graph_id = #{graph.id}) ORDER BY row, index ASC" }
end
