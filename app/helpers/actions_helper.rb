module ActionsHelper
  def action_to_html(action)
    ...
    when "User"
      ...
      when "show"
        "#{link_to action.user, action.user, :remote => remote? } viewed #{link_to_if action.target.class!='String', action.target, action.target, :remote => remote? }'s profile".html_safe
      when "destroy"
    ...
    when "Graph", "Node"
      ...
      when "update"
        "#{link_to action.user, action.user, :remote => remote? } edited #{link_to_if action.target.class!='String', action.target, action.target, :remote => remote? }".html_safe
      when "highlight"
        "#{link_to action.user, action.user, :remote => remote? } highlighted the group #{link_to_if action.target.class!='String', action.target, action.target, :remote => remote? }".html_safe
      when "unhighlight"
        "#{link_to action.user, action.user, :remote => remote? } unhighlighted the group #{link_to_if action.target.class!='String', action.target, action.target, :remote => remote? }".html_safe
      end
    end
  end
end
